...
rs.Open "Select * from users where userid='" & Trim(Text1.Text) & "'", co1, adOpenKeyset, adLockOptimistic
...
Set rs = New ADODB.Recordset
rs.Open "sELECT * FROM USERS", co1, adOpenKeyset, adLockOptimistic
If rs.RecordCount < 2 Then
MsgBox "Cant delete this record..at least one login is required", vbInformation, "Deletion Error"
Exit Sub
End If
...
Set rs = New ADODB.Recordset
rs.Open "delete   from USERS  where USERID='" & Trim(Text1.Text) & "'", co1, adOpenKeyset, adLockOptimistic
...
Set rs = New ADODB.Recordset
rs.Open "Select * from users order by userid", co1, adOpenKeyset, adLockOptimistic
If rs.RecordCount > 0 Then
rs.MoveLast
...
Else
rs.Open "Select * from users where utype <> 'Admin' order by username", co1, adOpenKeyset, adLockOptimistic
...
Set rs = New ADODB.Recordset
rs.Open "Select * from users where username='" & Trim(Text2.Text) & "'", co1, adOpenKeyset, adLockOptimistic
If rs.RecordCount > 0 Then
rs.MoveFirst
