...
Begin VB.Form dcustomrep 
   ...
   Begin VB.Label Label8 
      ...
      Caption         =   "Customer Wise Report(Special)"
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   12
         Charset         =   0
...
Grid1.Refresh
Grid1.Rows = 2
Grid1.FormatString = "|^          S.N.         |^    Date In   |^    Date Out   |^     Time In   |^    Time Out   |<   Customer Name |<   Destination        |<   RLW     |< DO No.|^  DO Date   |<Vehicle No.|<  Material   |<M_Code|<     coll. Code    |>   Ist Weight    |>    IInd Weight    |>      Net weight  |>    Cust Code   |>   Challan No  "
End Sub
...
If Trim(Text1.Text) <> "" Then
    SQL = SQL & " and tc_code = '" & Trim(Text1.Text) & "' order by sl_no desc"
Else
    SQL = SQL & " order by tc_code, sl_no desc"
End If
...
While Not rs3.EOF
    ...
    Grid1.TextMatrix(Grid1.Row, 18) = rs3.Fields("tc_code").Value
...
wstream.WriteLine Chr(27) & "E" & Chr(14) & PADC("Customer Wise Report(Sorted)", 28) & Chr(27) & "F"
Else
wstream.WriteLine Chr(27) & "E" & Chr(14) & PADC("Selected Customer Report", 28) & Chr(27) & "F"
End If
wstream.WriteBlankLines 1
...
If Grid1.Text <> matercod Then
    wstream.WriteLine Chr(18) & String(80, "-")
    wstream.WriteLine Chr(18) & Chr(15) & Chr(14) & Trim("customer ID: " & matercod) & "  Total: " & CStr(matertot)
    wstream.WriteLine Chr(18) & String(80, "-")
    matertot = 0
    materbr = False
End If
...
If Option1.Value = True Then
    spdatrep.Sections(2).Controls("repname").Caption = "Customer Wise Report (Sorted)"
Else
    spdatrep.Sections(2).Controls("repname").Caption = "Customer Wise Report (Selected)"
End If

spdatrep.Sections(2).Controls("repname").Caption = "From Date: " & CStr(DTPicker1.Value) & "      To Date: " & CStr(DTPicker2.Value)
If Option2.Value = True Then
    spdatrep.Sections(2).Controls("repname").Caption = spdatrep.Sections(2).Controls("repname").Caption + "     Customer: " & Text1.Text
End If
