VERSION 5.00
Begin VB.Form ItemMastsmp 
   Caption         =   "Simple Item Master"
   ClientHeight    =   10980
   ClientLeft      =   120
   ClientTop       =   420
   ClientWidth     =   15120
...
rs5.Open "delete  from  mater1 where m_code='" & Trim(Text1.Text) & "'", co, adOpenKeyset, adLockOptimistic
...
rs1.Open "select * from mater1 where m_code='" & Trim(Text1.Text) & "'", co, adOpenKeyset, adLockOptimistic
...
Set rs1 = New ADODB.Recordset
rs1.Open "Select * from mater1  order by val(m_code)", co, adOpenKeyset, adLockOptimistic
If rs1.RecordCount > 0 Then
rs1.MoveLast
...
Set rs = New ADODB.Recordset
rs.Open "Select * from mater1 order by M_CODE", co, adOpenKeyset, adLockOptimistic
If rs.RecordCount > 0 Then
rs.MoveFirst
End If
